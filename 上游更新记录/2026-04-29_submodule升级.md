...
> 这一节用大白话讲：上游作者这段时间在做什么？这次升级对你有什么实际影响？
> 想看技术细节请跳到下面的"详细变化"章节。

### MotorBridge 50 个 commit：作者在做 6 件事

#### 1. 项目正规化（最大动作）

**做了什么**：
- 把项目从个人账号 `tianrking/MotorBridge` 搬到组织账号 `motorbridge/motorbridge`
- 把 Studio UI（图形化调参界面）拆成独立项目 `motorbridge-studio`

**为什么**：作者准备把这个项目当成正式产品做。把 UI 和底层库拆开，**底层库只做电机通信，UI 是另一个项目**——职责清晰，将来谁想用底层库做自己的 UI 也方便。

**对你的影响**：将来想用图形界面要单独装 `motorbridge-studio`。

#### 2. 把达妙和 RobStride 电机支持做扎实 ⭐ 跟你直接相关

**做了什么**：
- 同步最新的达妙寄存器值（达妙厂商更新协议后跟进）
- RobStride 参数兼容性修复
- RobStride 模式切换稳定化（之前切换会崩）
- 多电机同时控制时给 CAN 总线发包加"节流"（避免打爆）
- 启停指令也加节流（`enable/disable burst`）

**为什么**：之前作者拿到的反馈是"多电机一起跑会丢帧"、"切模式偶尔崩"、"达妙寄存器对不上"。这一系列就是修这些。

**对你的影响**：
- 你现在正在调达妙电机，**直接受益**
- 将来 reBot 装好 7 个电机一起跑，也是直接受益

#### 3. macOS 支持

**做了什么**：加 PCBUSB CAN 接口支持 + 一键 setup 脚本 + macOS arm64 Python 包

**为什么**：之前 MotorBridge 在 macOS 上跑不起来。

**对你的影响**：你是 Windows，**用不上**。但说明项目越做越成熟。

#### 4. 新增独立 gateway 工具

**做了什么**：加了 `motorbridge-gateway` 入口（独立的 WebSocket 服务）。

**为什么**：让其他程序（Web 网页、其他语言写的程序）都能通过 WebSocket 远程调用电机。

**对你的影响**：现在用不上。**将来**做以下场景会用：
- 想用浏览器界面控制机械臂
- 想用其他语言（不是 Python）控制电机
- 想做远程控制

#### 5. 图形界面（Studio UI）大量增强

**做了什么**：实时拖动滑块控制电机、安全流程、重放工作流、序列库、帮助中心、风险弹窗。

**为什么**：让没编程基础的人也能调试电机。

**对你的影响**：你目前用 DMTool 调试，**这块基本用不上**。如果将来给别人演示 reBot，可以装这个 Studio。

#### 6. 文档和打包流程

**做了什么**：中英文文档对齐（mintlify）+ 修中文乱码 + 跨平台启动指引 + 自动化版本发布。

**对你的影响**：将来查文档更顺。

---

### StarArm_102 4 个 commit：作者在做 4 件事

#### 1. 优化 3D 模型 ⭐ 重要

**做了什么**：
- `link7_left.STL`：151 KB → 110 KB（变小 27%）
- `link7_right.STL`：229 KB → 204 KB（变小 11%）
- `link6.STL`：重新生成（大小没变但内容变了）

**为什么**：原模型可能太精细（顶点太多），渲染慢。**简化网格**让仿真和可视化更流畅。

**对你的影响**：将来用 MeshCat / Gazebo 做仿真时**加载更快**。

#### 2. 重写 URDF 文件 ⭐ 重要

**做了什么**：`stararm102_gazebo.urdf` 重写 216 行（关节、连杆描述）。

**为什么**：之前 URDF 可能有不准的地方（关节轴、惯量、坐标系），重写修正。

**对你的影响**：将来做遥操作时**主动臂坐标更准**，标定数据不会偏。

#### 3. 完善 ROS2 控制接口

**做了什么**：
- `ros2_controllers.yaml` 新增 14 行
- `moveit_controllers.yaml` 更新
- `gazebo_demo.launch.py` 调整

**为什么**：让 ROS2 + MoveIt2 能直接驱动这个主动臂。

**对你的影响**：将来做 ROS2 集成（reBot 路线图里的 🚧 进行中）时，主动臂这边的接口已经准备好。

#### 4. 关节限位说明 ⭐ 重要

**做了什么**：README 新增 Joint Limit 章节（每个关节能转多少度）。

**为什么**：没限位说明，做运动规划时容易撞限位损坏机械。

**对你的影响**：做遥操作或运动规划时，**必须看一下这个限位表**，做软件保护。
...
**MotorBridge 升级的核心收益**：
作者在**把项目从玩具变产品**——拆架构、稳定多电机、加 macOS、新增 gateway。
**对你最实在的好处**：达妙寄存器同步 + 多电机不丢帧 + 模式切换稳定。

**StarArm_102 升级的核心收益**：
作者在**修正模型和接口**——3D 模型变快、URDF 修准、ROS2 接口齐全、关节限位写清楚。
**对你最实在的好处**：将来做遥操作时**少踩坑**。
