...
> ⚠️ **重要：`usbipd` 是 Windows 命令，不是 Linux 命令！**
>
> 这是新手最容易踩的坑：在 WSL2 里跑 `usbipd list` 会报错 `Command 'usbipd' not found`。
>
> | 命令 | 运行位置 |
> |------|---------|
> | `usbipd list` / `bind` / `attach` / `detach` | ✅ **Windows PowerShell（管理员）** |
> | `lsusb` / `ls /dev/ttyACM*` / `python3 xxx.py` | ✅ **WSL2 Ubuntu** |

#### 5.0.1 安装 usbipd-win（只装一次）

**👉 Windows 管理员 PowerShell 里跑：**
...
**👉 Step 1 — Windows 管理员 PowerShell 里跑：**

```powershell
usbipd list
```

输出示例（**`2-3` 是举例，你要看自己实际输出**）：
```
Connected:
BUSID  VID:PID    DEVICE                                STATE
1-14   8087:0026  蓝牙                                  Not shared
7-2    1d50:606f  USB2CAN / CANable Adapter             Not shared    ← 就是这个！
```

找到 USB2CAN 设备，**记下它的 BUSID**（比如 `7-2`），然后：

```powershell
usbipd bind --busid 7-2             # ← 把 7-2 替换成你自己设备的 BUSID
usbipd attach --wsl --busid 7-2     # ← 同上
```

> ⚠️ **别照抄 `2-3`**，那是文档里的举例 BUSID。BUSID 因机器而异，必须先 `usbipd list` 看自己的。
> 常见 USB2CAN VID:PID：`1d50:606f`（CANable）、`0483:5740`（达妙）、`0403:6001`（FTDI 类）。

**👉 Step 2 — WSL2 Ubuntu 里跑，验证是否成功：**
...
**👉 Windows 管理员 PowerShell 里跑**（把 `7-2` 换成你自己的 BUSID）：
...
#### 5.0.4 跑 Python 代码的正确位置

**👉 WSL2 Ubuntu 里跑**（代码在 F 盘，通过 `/mnt/f/` 访问）：

```bash
# 进入代码目录（注意是 /mnt/f/ 不是 F:\）
cd /mnt/f/chengshenzhilu/Robot/reBot-DevArm/software/reBotArm_control_py

# 跑例程
python3 example/2_zero_and_read.py
```

> ⚠️ **不要在 `/mnt/c/Users/你的用户名/` 下跑**，那里没有代码，会报 `No such file or directory`。
