# 学习笔记

记录学习 reBot-DevArm 过程中的发现、踩坑和理解。

---

## 2026-06-01：机械臂摄像头——深度相机 vs 普通 RGB，以及具身大模型用哪个

> 背景：决定了引导臂做 102 后，开始想"机械臂还要配摄像头吧"，由此搞懂了一串关于相机选型和具身模型的原理。结论先记：**LeRobot AI 主线用普通 RGB USB 摄像头就够，不用买深度相机**。

### 1. 深度相机 vs 普通摄像头的本质区别

- **普通摄像头（webcam）**：只输出 RGB 彩色图——"一张照片"，不知道物体远近。
- **深度相机（RGB-D，如 Orbbec Gemini2 / Intel RealSense）**：RGB 彩色图 **+ 每个像素的距离（Depth）**——"照片 + 每个点的距离尺"，能凑出物体的 3D 坐标。
- 一句话：深度相机比普通摄像头多了"距离"这一维。

### 2. 关键发现：具身大模型（π0/ACT/SmolVLA）用的是 RGB，不是深度

这是最反直觉、也最重要的一点：

- 现在最火的 VLA（Vision-Language-Action）具身大模型——**π0 / π0.5 / π0.6（Physical Intelligence）、RT-2、OpenVLA、NVIDIA GR00T、LeRobot 自家 SmolVLA**——视觉输入**都是普通 RGB**。
- **为什么**：它们站在「视觉-语言大模型 VLM」（如 π0 底座 PaliGemma）肩膀上，而 VLM 是用**互联网海量 RGB 图片**预训练出来的，天生吃彩色图、不吃深度。
- **对我的意义**：用普通 USB 摄像头采的「几路 RGB 视频 + 关节动作时序」**正是喂这类大模型的标准数据格式**，LeRobot 已集成 pi0 可直接微调。→ 所以**不用花大钱买深度相机**。

### 3. 那为什么官方"视觉抓取 Demo"非要深度相机？

因为它走的是**传统 CV 路线，不是大模型路线**：

- demo 用 **YOLO**（`yoloe-26l-seg.pt` 开放词汇分割版）+ **OBB**（带方向最小外接矩形，短轴=夹爪开合方向）+ **Eye-in-Hand** 手眼标定。
- YOLO 是 **2D 模型**，只告诉你"物体在画面哪个像素 + 朝向"，**不知道距离**。深度相机补上"距离"→ 算出 3D 抓取点 → 手眼标定换成机械臂坐标 → IK → 抓。
- **对比顿悟**：传统 CV 要**显式**算几何（所以需要深度）；大模型是端到端学「RGB 像素 → 动作」，几何关系从海量 2D 演示里**隐式**学会（所以不需要深度）。这是"老办法"和"大模型办法"的根本分野。

### 4. 相机安装位置：eye-in-hand vs eye-to-hand

| 装法 | 位置 | 看到啥 |
|---|---|---|
| 腕部相机（Eye-in-Hand） | 装手腕，跟夹爪一起动 | 夹爪正前方近距离特写 |
| 俯视/固定相机（Eye-to-Hand） | 三脚架/夹子，不动，俯瞰桌面 | 整个工作台全景 |

- LeRobot 标准 = 2 个（腕部 + 俯视）；3 个略好但边际收益递减。
- 别买 2 个深度相机——第二个的"深度"LeRobot 根本不用，纯浪费。

### 5. ⚠️ 踩坑预警：两个同款 USB 摄像头会搞崩录制

社区血泪经验：**腕部和俯视千万别买两个一模一样的型号**——两个相同 USB 摄像头会导致 **USB 路径冲突，直接把 LeRobot 数据录制程序搞崩溃**。必须用两个不同厂家/型号。

### 学习收获

- "机械臂要配摄像头"这件事，**深度相机不是必需的**——这颠覆了我一开始"越贵越好/必须深度"的直觉。
- 看清了**传统 CV（YOLO+深度+几何）** 和 **具身大模型（RGB+端到端）** 是两条不同的路，对应不同的硬件需求。
- 选型落定：腕部小 USB 模组（配仓里 `UVC32_mount.step`）+ 俯视普通罗技，两个不同款。完整购物清单见 `遥操作与LeRobot待办.md` §3.9。

---

## 2026-05-05

### MotorBridge v0.2.4 的几个关键认知

#### 1. submodule 源码升级 ≠ Python 环境包升级

4-29 升级了 `software/MotorBridge` submodule 到 v0.2.4，但 Python 环境里还是 v0.1.7（4-13 装的）。**这两个是独立的**：
- submodule 源码 = git 管理的代码副本
- Python 环境包 = pip 安装的可执行库

**踩坑**：升级 submodule 后忘了升级 Python 包，跑示例时可能因为 API 不匹配报错。

**规则**：每次升级 submodule 后，必须重新 `pip install` 一次。

#### 2. MotorBridge 的 Python SDK 在子目录

主目录是 Rust 项目（`Cargo.toml`），Python SDK 在 `bindings/python/`：

```
software/MotorBridge/
├── Cargo.toml          ← Rust 主项目
├── motor_core/          ← Rust 核心
├── motor_vendors/       ← 厂商适配（达妙/RobStride）
├── motor_cli/
└── bindings/
    ├── python/          ← ⭐ Python SDK 在这里
    │   ├── pyproject.toml
    │   └── setup.py
    └── cpp/
```

正确安装：
```bash
pip install -e software/MotorBridge/bindings/python
```

如果只装预编译包：
```bash
pip install --upgrade motorbridge -i https://pypi.tuna.tsinghua.edu.cn/simple
```

#### 3. v0.2.4 的命名常量比裸数字好用

旧版本要写裸数字，新版有命名常量：

| 旧写法 | 新写法 |
|--------|--------|
| `motor.write_register_f32(25, ...)` | `motor.write_register_f32(motorbridge.RID_KP_ASR, ...)` |
| `motor.write_register_f32(28, ...)` | `motor.write_register_f32(motorbridge.RID_KI_APR, ...)` |

可用常量（`dir(motorbridge)` 看到的）：
- PID 参数：`RID_KP_ASR=25`（速度环 Kp）、`RID_KI_ASR=26`（速度环 Ki）、`RID_KP_APR=27`（位置环 Kp）、`RID_KI_APR=28`（位置环 Ki）
- 映射范围：`RID_PMAX=21`、`RID_VMAX=22`、`RID_TMAX=23`
- 通信配置：`RID_CTRL_MODE=10`（0x0A）、`RID_ESC_ID=8`、`RID_MST_ID=7`、`RID_TIMEOUT=9`

#### 4. 达妙专用安全工具

新版自带几个达妙专用集合：

| 名称 | 用途 |
|------|------|
| `DAMIAO_HIGH_IMPACT_RIDS` | 高风险寄存器（极对数、减速比等，写错会失控） |
| `DAMIAO_PROTECTION_RIDS` | 保护类寄存器（过压/过流/过温阈值） |
| `DAMIAO_RW_REGISTERS` | 全部可读写寄存器集合 |
| `get_damiao_register_spec(rid)` | 查询某个寄存器的规格（范围/类型） |

写参数前先查 `DAMIAO_HIGH_IMPACT_RIDS`，避免误改导致电机损坏。

#### 5. v0.2.4 没有 `__version__` 属性

旧习惯：
```python
import motorbridge
print(motorbridge.__version__)   # ❌ AttributeError
```

新做法（用 pip show）：
```bash
pip show motorbridge | head -5
```

或用 importlib：
```python
from importlib.metadata import version
print(version('motorbridge'))    # ✅ 通用方法
```

### example/1_damiao_text.py 是单电机 REPL 工具

**核心定位**：DMTool 的 Python 替代品，支持 4 种模式 + 寄存器读写 + 状态读取。

**优势**：
- 不依赖 GUI 工具，完全命令行
- 易于扩展（加自定义命令、自动化测试）
- 可作为后续编写整机控制代码的基础

**使用前必改**（第 31-34 行）：
```python
CHANNEL    = "/dev/ttyACM0"   # WSL2 保持 / Windows 改 "COM3"
MOTOR_ID   = 0x01              # 你电机的实际 CAN ID
FEEDBACK_ID = 0x11             # = 0x10 + MOTOR_ID
MODEL      = "4340P"           # ⚠️ reBot 小关节用 "4310"
```

**MODEL 不改的后果**：扭矩范围、PMAX 不一样，电机可能失控或扭矩输出错误。

---

## 2026-04-29

### 达妙电机的 4 种控制模式（重要）

DM-J4310-2EC V1.2 支持 4 种工作模式，**同一时间只能用一个**：

| 编码 | 模式 | 帧 ID 偏移 | 控制内容 |
|------|------|-----------|---------|
| 1 | MIT 模式 | CAN_ID | 位置+速度+力矩混合 |
| 2 | 位置速度模式 | CAN_ID + 0x100 | 三环串级控制 |
| 3 | 速度模式 | CAN_ID + 0x200 | 双环控制 |
| 4 | 力位混控模式 (PVT) | CAN_ID + 0x300 | 位置+力矩限制 |

**模式切换的两种方式**：
1. **串口"写参数"**（永久保存）：电机自动重启，模式存到 flash，掉电不丢失
2. **CAN 命令写寄存器 0x0A**（临时切换）：电机不重启，但掉电丢失

注意：模式切换会清零所有指令值（位置、速度、KP、KD 等），从其他模式切换到位置控制前，建议先读取当前位置（寄存器 0x50），并在零速时切换。

### MIT 模式的来源和含义

**MIT 模式**这个名字来源于**麻省理工学院**的 **MIT Cheetah 四足机器人**项目（2018 年左右开源），后来被全球电机厂商沿用。

**核心公式**：
```
T = Kp × (p_des - p_actual) + Kd × (v_des - v_actual) + t_ff
```

一个公式同时干 3 件事：位置控制 + 阻尼控制 + 前馈力矩。通过调参数可以衍生出多种控制方式：

| 想要的效果 | Kp | Kd | t_ff |
|-----------|-----|-----|------|
| 纯位置控制 | ≠0 | ≠0 | 0 |
| 纯速度控制 | 0 | ≠0 | 0 |
| 纯力矩控制 | 0 | 0 | ≠0 |
| 柔顺位置控制 | 小 | 小 | 0 |
| 重力补偿位置控制 | ≠0 | ≠0 | 重力力矩 |

**警告**：位置控制时 **Kd 不能为 0**，否则电机会震荡甚至失控。

**为什么机器人都用 MIT 模式**：柔顺性好（碰到障碍会"让一下"）、稳定性好（Kd 防震荡）、统一接口、响应快。

### PVT 模式 = 力位混控模式

达妙的 PVT 是 **P**osition + **V**elocity + **T**orque（不是工业控制里的 P-V-Time）：

| 字节 | 内容 | 类型 | 范围 |
|------|------|------|------|
| D[0-3] | P：位置 | float | rad |
| D[4-5] | V：限速值（×100） | uint16 | 0-10000（对应 0-100 rad/s） |
| D[6-7] | T：电流标幺值（×10000） | uint16 | 0-10000（对应 0-1.0） |

**应用场景**：抓取易碎物品、柔性装配、人机协作。比"位置速度模式"多了**力矩限制**，更安全。

### CAN 反馈帧的解析方法

收到的 8 字节反馈帧格式：

| 字节 | 含义 |
|------|------|
| D[0] | `ID|ERR<<4` 低 4 位是 ID，高 4 位是错误码 |
| D[1-2] | POS 位置（16 位有符号定点数）|
| D[3-4] | VEL 速度（12 位）|
| D[4-5] | T 扭矩（12 位）|
| D[6] | T_MOS 驱动板温度（℃）|
| D[7] | T_Rotor 电机温度（℃）|

**例**：反馈 `36 D1 4D 7F F7 FB 24 22`
- D[0] = 0x36 = `0011 0110` → 高 4 位 = 3（**错误码 = 3，输出轴校准异常**），低 4 位 = 6（ID）
- D[6] = 0x24 = 36℃（驱动温度）
- D[7] = 0x22 = 34℃（电机温度）

**错误码对照表**：

| 状态码 | 含义 |
|--------|------|
| 0 | 失能 |
| 1 | 使能（正常）|
| 3 | 输出轴校准异常 |
| 4 | 传感器输出异常 |
| 5 | 电机编码器校准异常 |
| 8 | 超压 |
| 9 | 欠压 |
| A | 过电流 |
| B | MOS 过温 |
| C | 电机线圈过温 |
| D | 通讯丢失 |
| E | 过载 |

### CAN 命令实测：让电机以速度模式转动

测试电机：DM-J4310-2EC，CAN ID = 1

**步骤**：
1. **使能**：帧 ID `0x201`，数据 `FF FF FF FF FF FF FF FC`
2. **速度命令（5 rad/s）**：帧 ID `0x201`，数据 `00 00 A0 40 00 00 00 00`
3. **必须勾选"定时发送"**（间隔 10 ms），否则电机因 CAN Timeout 自动失能
4. **停止**：数据 `00 00 00 00 00 00 00 00`
5. **失能**：数据 `FF FF FF FF FF FF FF FD`

**速度对应的 IEEE 754 浮点字节（低位在前）**：

| 速度 (rad/s) | 十六进制 |
|-------------|---------|
| 1.0 | `00 00 80 3F` |
| 5.0 | `00 00 A0 40` |
| 10.0 | `00 00 20 41` |
| 20.0（接近最大）| `00 00 A0 41` |
| -5.0（反向）| `00 00 A0 C0` |

**最大速度**：DM-J4310 24V 版本空载约 200 rpm = 20.94 rad/s。

### 踩坑：达妙电机调试工具版本不对，导致串口乱码、ID 错乱

**问题现象**：
1. 串口正常显示开机信息，但执行校准等操作时出现乱码
2. CAN 反馈帧的电机 ID 一会儿是 6，一会儿是 4，与设置的 0x01 不符
3. 读取寄存器参数（如控制模式 0x0A）发送 `00 00 33 0A 00 00 00 00`，但收到的是普通反馈帧而非参数返回
4. 校准时调试助手提示"数据传输失败"
5. 反馈帧 D[0] = 0x36，解析出 ERR=3（输出轴校准异常）

**排查过程**：
- 一开始以为是硬件问题（串口线、电源、屏蔽）
- 也怀疑是电机损坏或 ID 配置问题
- 改了波特率、降了发送频率都不行
- 最后发现是**工具版本问题**

**根本原因**：
使用了错误的调试工具版本 `Debugging_Tools_v.1.6.8.8.exe`，实际应该用 `DMTool v2.1.6.0-FD加速(1).exe`

**工具对比**：

| 工具 | 版本 | 是否可用 |
|------|------|---------|
| Debugging_Tools_v.1.6.8.8.exe | V1.6.8.8 | ❌ 乱码、通信不稳定 |
| DMTool v2.1.6.0-FD加速(1).exe | V2.1.6.0-FD | ✅ 正常 |

**关键结论**：
- 达妙电机说明书提到"V2.0.0.0 及以上版本"，但实际 V1.x 系列工具完全不兼容新固件
- 必须用 **V2.x 系列**的 DMTool，且最好用带 **FD加速**的版本
- 工具位置：`hardware/测试工具/DMTool v2.1.6.0-FD加速(1).exe`

**怎么避免再踩坑**：
- 拿到达妙电机后，先确认工具版本 ≥ V2.0
- 出现以下症状立刻怀疑工具版本：
  - 串口乱码（特别是只在调试操作时乱码）
  - CAN 反馈 ID 与配置不一致
  - 读取参数无响应
  - 校准提示"数据传输失败"
- 不要花时间排查硬件，先换工具试

### Git 进阶操作（今天踩坑学到的）

#### 1. 中文 commit message 必须用文件，不能用 HEREDOC

**踩坑**：用 `git commit -m "$(cat <<'EOF' ... EOF)"` 提交中文 message 时，Windows shell 会出现字符截断乱码（"快照" → "快�"，"永不变动" → "永不变��"）。

**正确做法**：
```bash
# 1. 把 message 写到临时文件
# （可以用 Write 工具写或者 cat > 重定向）
# 2. grep "�" 验证文件无乱码
# 3. git commit -F .git/COMMIT_MSG_TEMP.txt
# 4. rm 删除临时文件
# 5. git log -1 --format=%B 验证 commit message 正常
```

**Why:** Windows 下 shell 处理长中文 HEREDOC 会有 buffer 截断问题，写文件再读取最稳。

#### 2. 撤销刚 commit 的：reset --soft 而不是 --amend

发现 commit message 写错了（乱码）时：
- ❌ 不要用 `git commit --amend`（项目规则禁止 amend，只创建新 commit）
- ✅ 用 `git reset --soft HEAD~1` 撤回，保留所有改动，重新 commit

**Why:** amend 会改写历史，对已 push 的 commit 危险。reset --soft + 重 commit 习惯更稳。

#### 3. submodule 切换到自己 fork 的完整流程

今天把 MotorBridge 和 StarArm_102 从原作者直连切到 fanhao375 fork，完整流程：

```bash
# (1) 改 .gitmodules（手动编辑或用 git config）
git config -f .gitmodules submodule.<path>.url <新URL>

# (2) 同步到 .git/config
git submodule sync

# (3) 进 submodule 内改 origin + 加 upstream
cd <submodule>
git remote set-url origin <fork URL>
git remote add upstream <原作者 URL>

# (4) 验证连通性
git fetch origin && git fetch upstream

# (5) 回主仓库 commit .gitmodules
cd ..
git add .gitmodules && git commit
```

**关键认知**：
- submodule 锁定的 commit 不变（fork 是从同一 commit 创建的）
- `.git/config` 里的 submodule URL 变化是本地的，不入 git
- `.gitmodules` 才是入库的唯一权威

#### 4. baseline tag 是复刻工程的"快照"

第一次理解 git tag 在复刻工程里的作用——不是发版本号，是**锁定一个已验证的状态**。

```bash
git tag -a baseline-2026-04-29 -m "复刻基线 2026-04-29
四个核心仓库的 submodule 锁定状态：
- ...
"
```

将来不管 main 怎么演进，`git checkout baseline-2026-04-29` 都能回到这个已知好的状态。

**Why:** 复刻工程不是"追最新"，是"维护一套可回滚的版本基线"。tag 是这个基线的物化体现。

#### 5. commit 必须分层

今天把 9 个 commit 分得很清楚：
- 整理电机文档
- 确立维护原则
- 修正核心定位
- 更新上游地址
- 切换 submodule
- 补充 Git 指南
- ...

**反面教材**：把"整理文档 + 修策略 + 升级 submodule + 改 .gitignore"塞一个 commit 里。

**Why:** 以后用 `git bisect` 定位 bug、用 `git revert` 回滚某个变更时，分层 commit 能精准定位到一类工作。

#### 6. 项目同步策略：检查文档是主依据，AI 记忆是辅助

今天讨论同步策略时一个关键修正：**仓库文档（`复刻基线维护原则.md`、`项目总览.md`）是项目最高依据**，AI 记忆只能做辅助指针。

**Why:**
- AI 记忆有过期标记（5 天就提示 may be outdated）
- 不同 AI 之间记忆不互通
- 用户和协作者看不到 AI 记忆
- 仓库文档才是真正的项目交付物

如果 AI 记忆和仓库文档冲突，永远以文档为准。

---

## 2026-04-12

### 项目认识

- reBot-DevArm 是 SeeedStudio 的开源 6DOF 机械臂项目，目标是降低具身智能学习门槛
- 有两个版本：B601 DM（达妙电机）和 B601 RS（RobStride 电机），外观结构一样，只是电机不同
- 截至今天，仓库里硬件资料齐全，软件代码刚发布（Python SDK + Pinocchio）

### 电机相关

- 两款电机都是**驱动一体化**（电机+FOC驱动器+双编码器集成在一个壳里），不需要单独买驱动器
- DM4340P（大电机）：27Nm 峰值扭矩，减速比 40:1，用在底座/肩部/大臂
- DM4310（小电机）：7Nm 峰值扭矩，减速比 10:1，用在小臂/腕部
- 4340P 比 4310 多一个"力位混控模式"，可以同时控制位置和限制力矩
- 所有电机通过 CAN 总线 @ 1Mbps 菊花链串联，协议是基于 MIT Mini Cheetah 的私有协议，不是 CANopen
- 反馈 ID 规则：feedback_id = motor_id + 0x10

### 通信协议

- 这台臂用的是达妙私有 CAN 协议（MIT 兼容），不是工业标准 CANopen
- 对于 7 个关节的桌面机械臂，CAN 总线完全够用
- 人形机器人（20+关节）才需要 EtherCAT，那是另一个层级的东西
- 达妙电机支持 CAN FD（波特率 > 1Mbps 自动切换），但默认 CAN 2.0B

### 供电

- 24V / 14.6A / 350W 电源，日常够用
- 7 个电机理论峰值总功耗 1296W，但不可能同时满载
- 正常工作 100-200W，高负载 200-300W

### 性能瓶颈

- 2 号电机（肩部 DM4340P）是瓶颈，长时间高负载容易过热
- 推荐：负载 ≤ 1.5kg，臂展 < 70%，连续工作 2 小时休息 15 分钟
- 建议加主动散热

### 环境踩坑

- `pyproject.toml` 里 `requires-python = "==3.10.*"` 太严格，改成 `>=3.10` 就能用 3.12
- **pip install pin 在 Windows 上会卡死**，因为要从源码编译 C++ 库
- 解决方案：用 WSL2（Ubuntu）或 conda 安装，有编译好的二进制包
- WSL2 默认装 C 盘，但可以通过 import/export 方式装到 F 盘

### 待做

- [ ] 安装 WSL2 Ubuntu 到 F 盘
- [ ] 在 WSL2 里装 Python + Pinocchio 环境
- [ ] 跑通正运动学测试（example/5_fk_test.py）
- [ ] 跑通逆运动学测试（example/6_ik_test.py）
- [ ] 深入读 reBotArm_control_py 源码

---

## 2026-04-13

### WSL2 安装踩坑

- `pip install pin` 在 Windows 上从源码编译 C++，会卡死不动，根本装不上
- 解决方案：用 WSL2 Ubuntu，Linux 下 pip 有预编译的二进制包
- WSL2 默认装 C 盘，但 C 盘空间不足，需要装到 F 盘
- **装到 F 盘的方法**：不能直接指定路径安装，需要先下载 appx 包 → 解压 → 找到 install.tar.gz → 用 `wsl --import` 指定 F 盘路径导入
- PowerShell 的 `curl` 不是真 curl，要用 `Invoke-WebRequest`
- PowerShell 默认不走 VPN 代理，需要手动设置：`$env:HTTP_PROXY="http://127.0.0.1:7890"` + `-Proxy` 参数
- 微软 appx 包里套了两层：外层 appx 里有 ARM64 和 x64 两个子 appx，要解压 x64 那个才能找到 install.tar.gz
- 进入 WSL 后提示代理警告（localhost proxy not mirrored），不影响使用

### 正运动学测试通过

- 跑通了 `example/5_fk_test.py`
- 归零位置 (0 0 0 0 0 0) 末端：X=0.253m, Y=0, Z=0.172m
  - 约 25cm 正前方、17cm 高，姿态几乎为单位矩阵，说明 URDF 模型正确
- 自定义角度 (1 25 30 45 60 79) 末端：X=0.135m, Y=-0.150m, Z=-0.088m
  - 末端往下往右移了，姿态有旋转，符合预期

### 待做
- [x] WSL2 里装 Python + Pinocchio
- [x] 跑通正运动学测试
- [x] 跑通逆运动学测试（修了官方 bug）
- [x] 跑通 MeshCat 可视化三件套
- [ ] 读运动学源码理解 Pinocchio 调用方式
- [ ] 深入读 reBotArm_control_py 源码
- [ ] 等 04.20 Isaac Sim 官方教程发布

### 逆运动学测试 — 官方示例有 bug

- `example/6_ik_test.py` 调用 `compute_ik()` 传的参数名全是错的
- 说明 develop 分支的示例代码和库接口还没对齐，代码还在开发中
- 修复后验证：输入 FK 归零坐标 (0.253, 0, 0.172) → IK 解出 (0,0,0,0,0,0)，正确
- 收敛状态显示"否"，但误差只有 3.80e-04（阈值 1e-4），实际已经非常接近，结果可用
- **教训**：开源项目的 develop 分支不一定稳定，跑之前要有心理准备会遇到 bug

### MeshCat 可视化三件套（无硬件学习的最佳工具）

#### 环境问题：WSL2 访问 Windows 文件的 Git symlink 失效

- **现象**：`python3 example/sim/fk_sim.py` 报错 `Mesh package://reBot-DevArm_description_fixend/meshes/base_link.STL could not be found`
- **原因**：仓库里 `urdf/reBot-DevArm_description_fixend` 是指向 `reBot-DevArm_fixend_description` 的软链接（两个目录名只是单词顺序不同）
  - Linux/Mac 下 git clone 会还原成真·软链接 → Pinocchio 正常工作
  - **Windows 下 Git 把软链接存成了纯文本文件**（31 字节，内容就是目标路径字符串）
  - WSL2 通过 `/mnt/f` 访问 Windows NTFS 文件系统时，看到的是那个文本文件，不是目录
- **解决方案**：在 WSL2 里手动重建软链接
  ```bash
  cd urdf
  rm reBot-DevArm_description_fixend
  ln -s reBot-DevArm_fixend_description reBot-DevArm_description_fixend
  ```
- **验证**：`ls -la` 看到 `lrwxrwxrwx ... reBot-DevArm_description_fixend -> reBot-DevArm_fixend_description` 就对了
- **教训**：WSL2 + Windows Git 混用时，软链接是个坑；遇到"文件找不到"先检查是否是 symlink 问题

#### 1. fk_sim.py — 正运动学交互式可视化

- **功能**：输入 6 个关节角（度）→ 浏览器里实时显示机械臂 3D 姿态
- **启动**：`python3 example/sim/fk_sim.py` → 终端打印 `http://127.0.0.1:7000/static/` → Windows 浏览器打开
- **操作**：
  - 输入 `0 0 0 0 0 0` → 归零姿态（竖直）
  - 输入 `0 -90 0 0 0 0` → J2 弯 90 度（大臂水平）
  - 输入 `45 -30 15 -60 90 180` → 六关节复杂姿态
- **浏览器操作**：左键拖 = 旋转视角，右键拖 = 平移，滚轮 = 缩放
- **价值**：把光秃秃的 FK 坐标数字变成肉眼可见的 3D 模型，直观验证运动学正确性

#### 2. ik_sim.py — 逆运动学交互式可视化（已改进）⭐

- **功能**：输入目标位置 (x, y, z) 或位姿 (x, y, z, roll, pitch, yaw) → 机械臂自动求解关节角并摆到目标位置
- **启动**：`python3 example/sim/ik_sim.py`
- **原版问题**：
  - 代码里只调用了 `viz.update(result.q)`，**没调用 `viz.show_ik_pose()` 显示目标点标记**（红球 + 三色坐标轴）
  - 每次从零位 `[0,0,0,0,0,0]` 开始求解（`q_init=None`），不使用上一次的解作为初值
  - 这导致很多点容易陷入局部最优 → 报 `[未收敛]`
  - 例如 `(0.25, 0.0, 0.15)` 误差 2.15cm，`(0.2, 0.0, 0.25)` 误差 9.46cm
- **改进方案**（已实施）：
  1. **添加目标点可视化**：调用 `viz.show_ik_pose(target_pos, target_rot, result.q)` 显示红球 + 坐标轴
  2. **连续初值策略**：保存 `q_current`，每次从上一个解出发求解下一个目标
  3. **调试输出增强**：解析失败时显示原始输入和分割结果
- **改进后测试结果**：
  - `0.25 0.0 0.2` → ✅ 收敛，9 次迭代，误差 0.056mm
  - `0.3 0.0 0.25` → ✅ 收敛，10 次迭代，误差 0.069mm（J2 -32°, J3 -18°）
  - `0.2 0.15 0.3` → ✅ 收敛，12 次迭代，误差 0.063mm（J1 腰转 81°）
  - **三个点全部收敛**，浏览器里红球正确显示并跟随移动
- **价值**：改进后成为真正可用的交互式 IK 调试工具，适合快速验证单点 IK 求解

#### 3. traj_sim.py — SE(3) 测地线轨迹规划 + CLIK 跟踪（最有料）⭐⭐⭐

- **功能**：输入目标位姿 → 规划 SE(3) 测地线轨迹 → CLIK 逐帧跟踪 → 浏览器动画回放
- **启动**：`python3 example/sim/traj_sim.py` → 显示当前末端位置 `pos[0.253 0.000 0.172]`
- **测试 1（简单平移）**：
  - 输入：`0.3 0.0 0.25`（往前 + 往上）
  - 结果：IK 成功率 96.1%，最大误差 0.34mm，平均误差 0.05mm
  - 关节运动：J2 -32.3°, J3 -17.7°, J4 -14.6°（肩/肘/腕弯曲）
  - 轨迹：51 点，1.0 秒，dt=0.02s
- **测试 2（复杂姿态变化）**：
  - 输入：`0.2 0.15 0.35 0 0.5 0.3`（位置 + pitch 0.5 + yaw 0.3 弧度）
  - 结果：**IK 成功率 100%** 🎉，最大误差 0.099mm，平均误差 0.057mm
  - 关节运动：**6 个关节全动**，J1 转 63.4°（腰转），J5/J6 转腕
  - 轨迹：121 点，2.4 秒
- **测试 3（超出工作空间）**：
  - 输入：`0.15 0.2 0.3`
  - 结果：`IK 无解` ❌
- **浏览器画面**：
  - **灰色路径线**：规划的参考轨迹（SE(3) 测地线）
  - **绿色路径线**：实际跟踪轨迹（CLIK 输出）
  - **机械臂动画**：沿轨迹逐帧播放，肉眼验证跟踪精度
- **价值**：
  - 这是 `reBotArm_control_py_运行流程.drawio` 里**路径 A（move_to_traj）的无硬件版**
  - 完整展示了 IK → SE(3) 测地线 → CLIK → 动画播放 的全流程
  - 跟踪误差 < 0.1mm，说明 Pinocchio + CLIK 算法实现质量很高

#### 关键收获

1. **MeshCat = 无硬件版的"数字孪生"**，浏览器里实时 3D 预览机械臂运动
2. **三个脚本的定位**：
   - `fk_sim.py`：最简单，验证 FK 正确性
   - `ik_sim.py`：有 bug，跳过
   - `traj_sim.py`：最有料，完整展示轨迹规划 + 跟踪
3. **工作空间限制**：推荐半径 < 450mm（70% reach），超出会 IK 无解
4. **零位末端位置**：`(0.253, 0.000, 0.172)` m，约 25cm 正前方、17cm 高
5. **下一步**：等 04.20 Isaac Sim 官方教程发布，到时候已经有 Pinocchio + URDF 基础，上手会很快


---

## 2026-04-16

### 重力补偿深入学习

#### 1. 安装可视化工具集

- 克隆 `markdown-viewer/skills` 仓库到 `/tmp/`
- 软链接 4 个 skills 到 `~/.claude/skills/`:
  - `uml` — PlantUML 类图/序列图/活动图
  - `architecture` — HTML/CSS 分层架构图
  - `infographic` — YAML 流程图/时间线/对比图
  - `mindmap` — PlantUML 思维导图
- **目的**: 以后读代码时可以快速生成可视化图表辅助理解

#### 2. 重力补偿代码解析

- **核心文件**: `example/9_gravity_compensation.py` (基础漂浮版)
- **控制律**: `τ = g(q) + kp·(q_target - q) + kd·(0 - q̇)`
  - `g(q)` — 重力补偿项 (前馈)
  - `kp·Δq` — 位置误差反馈 (刚度)
  - `kd·Δq̇` — 速度阻尼 (避免震荡)
- **效果**: 机械臂漂浮在空中,松手不掉,轻推会移动
- **底层**: Pinocchio 的 `computeGeneralizedGravity()` 函数,基于 RNEA 算法

#### 3. 代码架构梳理

生成了 3 张可视化图表:

**3.1 分层架构图** (`architecture_代码架构.md`)


**3.2 函数调用序列图** (`sequence_函数调用序列.md`)
- 用户代码 → 500Hz 控制循环 → `compute_generalized_gravity()` → `pin.computeGeneralizedGravity()` → RNEA 算法
- 完整展示了从业务代码到底层算法的调用链

**3.3 控制循环流程图** (`control_loop_控制循环流程.md`)
- 5 个步骤: 读取关节位置 → 计算重力补偿 → MIT 控制律 → 打印力矩 → 循环
- 频率: 500Hz (2ms 周期)

#### 4. 文档整理

- 新建 `software/docs/02_reBotArm_control_py/重力补偿详解/` 目录
- 生成 4 个文档:
  - `README.md` — 完整教程 (7789 字节)
    - 什么是重力补偿 (问题 + 解决方案)
    - 物理原理 (动力学方程 τ = M·q̈ + C·q̇ + g(q))
    - 数学推导 (RNEA 算法 + MIT 控制律)
    - 代码架构 (4 层架构)
    - 代码详解 (核心代码逐行解释)
    - 实战示例 (9 号和 10 号例程)
    - 常见问题 (FAQ)
  - `architecture_代码架构.md` — 4 层架构图
  - `sequence_函数调用序列.md` — UML 序列图
  - `control_loop_控制循环流程.md` — Infographic 流程图
- 更新 `software/docs/README.md` 和 `software/docs/02_reBotArm_control_py/README.md` 索引

#### 5. 同步 upstream 更新

- 配置 git 代理 (端口 10808)
- `git fetch upstream` 发现 2 个新提交:
  - `b876c69` Merge pull request #15
  - `c3cee75` Create DISCLAIMER.md (免责声明文档)
- **DISCLAIMER.md 要点**:
  - 产品定位: DIY 套件,需自行组装调试
  - 安全风险: 高速高扭矩,可能夹伤/撞击/砸伤
  - 责任划分: 厂家只对出厂材料缺陷负责
  - 禁止场景: 医疗/生命支持/载人/水下/工业高危等
  - 免保条款: 拆机/改装/进水/过保/无发票等不保修

#### 关键收获

1. **重力补偿 = 前馈控制**: 提前算出重力力矩 `g(q)`,让电机不用对抗重力
2. **RNEA 算法**: Recursive Newton-Euler Algorithm,O(n) 复杂度,适合实时控制
3. **MIT 控制律**: `τ = g(q) + kp·Δq + kd·Δq̇`,简单但有效
4. **代码分层清晰**: 业务层 → 封装层 → 计算层 → 底层,易于理解和维护
5. **可视化工具**: 以后读代码可以快速生成架构图/序列图/流程图

#### 下一步

- 继续学习 10 号例程 (进阶锁止版):
  - 末端速度锁止 (v_ee > 0.04 或 w_ee > 0.08 时更新目标)
  - 积分项 (抵消重力补偿误差)
  - 更高刚度 (kp=8, kd=1.5)
- 深入 Pinocchio 的 `computeGeneralizedGravity()` 实现
- 理解动力学模型的 M、C、g 三项

#### 6. 踩坑：Markdown 流程图渲染器选型

**问题**：用 markdown-viewer/skills 生成的 3 个图，在 VS Code 里只有 `architecture_代码架构.md`（HTML/CSS）能直接看，另外两个都是空白：
- `sequence_函数调用序列.md` 用 PlantUML（` ```plantuml `）→ 不渲染
- `control_loop_控制循环流程.md` 用 Infographic（` ```infographic `）→ 不渲染

**原因**：
| 引擎 | VS Code 原生支持 | 备注 |
|------|----------------|------|
| HTML/CSS | ✅ 是 | universal，任何渲染器都行 |
| Mermaid | ✅ 是（已装 `bierner.markdown-mermaid`） | GitHub/Typora 也原生支持 |
| PlantUML | ❌ 否 | 需要 `jebbs.plantuml` 插件 + Java + Graphviz |
| Infographic | ❌ 否 | YAML 自定义语法，只有特定浏览器扩展能渲染 |

**解决**：把两个不渲染的图都改成 mermaid 格式
- `sequenceDiagram` 替代 PlantUML 序列图（保留 actor / participant / activate / Note）
- `flowchart TD` 替代 infographic 流程图（用 `classDef` 着色）

**关键收获**：
1. **选图表引擎要先看渲染器**：图再漂亮，渲染不出来等于零
2. **mermaid 是最稳的跨平台选择**：VS Code、GitHub、Typora、Obsidian 都原生支持
3. **HTML/CSS 是 fallback**：复杂图（带颜色块、装饰）走 HTML/CSS 最稳
4. **PlantUML 适合本地工具链**：如果要导出 PNG/SVG 用 PlantUML 更强，但写在 md 里看就别用了

#### 7. 踩坑：上游 `get_gravity()` 函数与 Pinocchio 3.9 不兼容

**症状**：跑 `example/sim/gravity_sim.py` 报错
```
AttributeError: 'numpy.ndarray' object has no attribute 'x'
  File "...dynamics/robot_model.py", line 133, in get_gravity
    return np.array([g.linear.x, g.linear.y, g.linear.z])
```

**原因**：上游代码（`reBotArm_control_py/dynamics/robot_model.py:133`）写的是
```python
return np.array([g.linear.x, g.linear.y, g.linear.z])  # 假设 .linear 有 .x .y .z
```
这套 API 是**老版 Pinocchio**（2.x 时代）的，`pin.Motion.linear` 返回 `pin.Vector3`，可以用 `.x/.y/.z` 取分量。

**新版 Pinocchio 3.9.0**（CHANGELOG 2026-04-13 第 2 条装的版本）已经把 `pin.Motion.linear` 改成直接返回 `numpy.ndarray`，没有 `.x` 属性了。所以上游函数在新版 Pinocchio 上必崩。

**正确写法**（任一种都行）：
```python
g_vec = np.asarray(model.gravity.linear)              # 最简
g_vec = np.array([g.linear[0], g.linear[1], g.linear[2]])  # 索引访问
```

**怎么处理的**：
1. **没动上游库** —— 改了就和官方分叉，将来同步麻烦
2. **在我们自己的 `gravity_sim.py` 里绕开**：把 `get_gravity(model)` 调用换成 `np.asarray(model.gravity.linear)`，记一行注释说明原因
3. **可选**：将来给上游提 issue/PR

**关键收获**：
1. **Pinocchio 2.x → 3.x 有 API breaking change**：老代码（特别是 GitHub 上 2-3 年前的项目）很可能踩
2. **类似踩坑可能还有**：所有形如 `motion.linear.x / motion.angular.x / placement.translation.x` 的代码，在新版都要改成索引或 numpy
3. **遇到外部库 bug 的处理顺序**：能在自己代码里绕开 → 绕开（不动上游）；不能绕开 → 提 issue + 本地 monkey patch；最后才考虑 fork 改源码
4. **类比踩过的坑**：之前 `6_ik_test.py` 也是上游 bug（参数名不匹配），那次直接改了 example 文件（属于 example 不是核心库，改了不影响同步），这次是核心库（`dynamics/`），就不动它

#### 8. 踩坑：仿真不会自动检查关节限位（用户发现 ⭐）

**现场**：跑 `gravity_sim.py` 时输入 `0 -45 90 0 0 0` 和 `0 -60 60 -30 30 0`，看到 j2 暴增到 -8.7 N·m，CC 直觉地问"是不是有些超脱了物理极限呀，有些B它并不能那么弯折吧"。

**结论：CC 直觉对，这两个姿态真机做不到。**

**URDF 里 reBot-DevArm 的关节限位**（`urdf/reBot-DevArm_fixend.urdf`）：

| 关节 | URDF 字段 | 实际范围 | 备注 |
|------|----------|---------|------|
| j1 base_yaw       | `[-3.14, +3.14]` | -180° ~ +180° | 全圈 |
| **j2 shoulder**   | `[-3.14, **0**]`  | **-180° ~ 0°** | ⚠️ 只能往下 |
| **j3 elbow**      | `[-3.14, **0**]`  | **-180° ~ 0°** | ⚠️ 只能往下 |
| j4 wrist_roll     | `[-1.87, +1.57]`  | -107° ~ +90°   | |
| j5 wrist_pitch    | `[-1.57, +1.57]`  | -90° ~ +90°    | |
| j6 end_roll       | `[-6.28, +6.28]`  | -360° ~ +360°  | 多圈 |

**关键**：**j2 / j3 的 upper = 0**，意味着这两个关节**只能往负方向弯**。这不是 URDF 配错，是真实机械限制——大臂和小臂背后有挡块、肘关节有偏置盖板，物理上不允许往反方向折。

**为什么超限会让 τ_g 暴涨？**
仿真把机械臂掰成了真机不可能的姿态，质心被甩到肩关节后方，杠杆变长，力矩自然爆表。

**Pinocchio 的默认行为**：
- `pin.forwardKinematics()` / `pin.computeGeneralizedGravity()` **不会检查限位**，给什么 q 算什么
- 限位信息存在 `model.lowerPositionLimit` / `model.upperPositionLimit`（np.ndarray，弧度）
- 用户代码自己负责 validate

**修复方案**（已应用到 `gravity_sim.py`）：
1. 启动时打印所有关节的 URDF 限位
2. 每次输入后用 `check_joint_limits()` 检查，超限**只警告不阻止**（仿真允许"看一下超限会怎样"是有学习价值的）
3. 推荐姿态全部改成满足限位的版本：
   ```
   0 0 0 0 0 0
   0 -90 0 0 0 0
   0 -45 -90 0 0 0      # 原来错写成 +90，肘要往后收
   0 -60 -60 30 30 0    # 原来错写成 +60
   0 -120 -30 0 0 0
   ```

**更深层收获**：
1. **运动学/动力学计算都不带限位检查**——这是工业惯例（计算和约束分离）
2. **真机 vs 仿真的鸿沟**：仿真能算的，真机未必做得到（限位、碰撞、奇异点）
3. **直觉是好滤镜**：当数据"看起来不对劲"时，先怀疑物理边界，再怀疑算法
4. **效率对比工具**：URDF 里 `effort` 字段是单关节扭矩上限（j2/j3 = 27 N·m，j4/j5/j6 = 7 N·m）。下次可以加一个"超 effort 警告"，提示这个姿态真机也撑不住

---

## 2026-04-23：10 号算法——末端速度锁止版重力补偿

### 这个算法是什么？

`10_gravity_compensation_lock.py` 是一个**拖动示教控制器**，让机械臂在重力补偿的基础上实现"轻推能动、停手就锁"的效果。

**核心思路**：
- 持续计算末端执行器的**线速度 v_ee** 和**角速度 w_ee**
- 当末端速度 **低于阈值** 时：目标关节角度 `q_target` 保持锁定（被推也纹丝不动）
- 当末端速度 **超过阈值** 时：目标关节角度 `q_target` 更新为当前关节角度 `q`（用力推才能换位置）

**控制律**（MIT 模式）：
```
τ = τ_g(q) + kp·(q_target - q) + kd·(0 - q̇) + I
```

其中：
- `τ_g(q)` = 重力补偿力矩（Pinocchio 计算）
- `kp·(q_target - q)` = 位置误差反馈（kp=8.0）
- `kd·(0 - q̇)` = 速度阻尼（kd=1.5，目标速度为 0）
- `I` = 积分项（误差累积修正，限幅 ±0.5 N·m）

---

### 解决什么问题？

**问题 2：纯重力补偿（09 号）的缺陷**
- 09 号算法只抵消重力，没有位置锁定，机械臂会"飘"（微小扰动就漂移）
- 用户松手后，机械臂不会保持在当前位置

**10 号的解决方案**：
- 用**末端速度**作为"用户正在推"的判据（而不是关节力矩或位置误差）
- 速度超阈值 → 认为用户在推 → 更新 `q_target` 跟随当前位置
- 速度低于阈值 → 认为用户松手 → 锁定 `q_target` 不变

这样就实现了"轻推能动、停手就锁"。

---

### 怎么用？

**硬件要求**：
- 机械臂已连接并使能
- 电机支持 MIT 模式（达妙 DM-J4310-2EC 支持）

**运行**：
```bash
cd software/reBotArm_control_py
python example/10_gravity_compensation_lock.py
```

**预期行为**：
1. 启动后，机械臂锁定在当前位置（`q_target` = 初始关节角度）
2. 用力推末端执行器 → 末端速度超过阈值（0.04 m/s 或 0.08 rad/s）→ `q_target` 更新为当前角度 → 机械臂跟随你的推动
3. 松手 → 末端速度降到阈值以下 → `q_target` 锁定 → 机械臂保持在当前位置
4. Ctrl+C 停止

**终端输出示例**：
```
[  20] LOCKED  err=+0.001  +0.002  -0.001  +0.000  +0.001  -0.000  Int=+0.001  +0.002  -0.001  +0.000  +0.001  -0.000  v=0.0012m/s  w=0.0034rad/s  tau_g=+0.234  -1.456  +0.789  -0.123  +0.045  +0.012  N·m
[  40] UPDATE  err=+0.123  +0.234  -0.089  +0.045  +0.012  -0.003  Int=+0.111  +0.211  -0.080  +0.041  +0.011  -0.003  v=0.0523m/s  w=0.1234rad/s  tau_g=+0.245  -1.478  +0.801  -0.125  +0.046  +0.013  N·m
```

- `LOCKED`：末端速度低于阈值，目标角度锁定
- `UPDATE`：末端速度超过阈值，目标角度跟随当前角度
- `err`：位置误差 `q_target - q`（rad）
- `Int`：积分项（N·m）
- `v`：末端线速度（m/s）
- `w`：末端角速度（rad/s）
- `tau_g`：重力补偿力矩（N·m）

---

### 关键代码解析

#### 1. 末端速度计算（L105-110）

```python
pin.computeJointJacobians(_model, _data, q)
pin.updateFramePlacements(_model, _data)
J = pin.getFrameJacobian(_model, _data, _ee_frame_id, pin.ReferenceFrame.WORLD)
v_spatial = J @ qd
v_ee_norm = float(np.linalg.norm(v_spatial[:3]))
w_ee_norm = float(np.linalg.norm(v_spatial[3:]))
```

- `J` 是末端执行器的**雅可比矩阵**（6×6），把关节速度 `q̇` 映射到末端空间速度
- `v_spatial = J @ qd` 得到 6D 空间速度：前 3 维是线速度 `v_ee`，后 3 维是角速度 `w_ee`
- 计算速度的**范数**（模长）作为判据

#### 2. 锁止逻辑（L112-117）

```python
if v_ee_norm > _VEL_THRESHOLD or w_ee_norm > _W_VEL_THRESHOLD:
    _q_target = q.copy()
    _lock_counter = 0
    _integral *= 0.9  # 速度超阈值时积分衰减，避免锁止后误差积累
else:
    _lock_counter += 1
```

- 速度超阈值 → 更新 `q_target` 为当前角度 `q`（跟随）
- 速度低于阈值 → 保持 `q_target` 不变（锁定）
- 积分项在速度超阈值时衰减 0.9 倍，避免锁止后误差积累导致抖动

#### 3. 积分项修正（L98-103）

```python
if _integral is None:
    _integral = np.zeros_like(q)

_integral += q_error * 1.0   # 积分增益 1.0，每个控制周期累积
np.clip(_integral, -0.5, 0.5, out=_integral)  # 限幅
```

- 积分项用于修正重力补偿的**静态误差**（比如 URDF 质量参数不准、摩擦力）
- 每个控制周期（2ms）累积一次：`_integral += q_error * 1.0`
- 限幅 ±0.5 N·m，避免积分饱和

#### 4. MIT 模式控制（L119-125）

```python
arm.mit(
    pos=_q_target,
    vel=np.zeros(arm.num_joints),
    kp=np.full(arm.num_joints, _KP),
    kd=np.full(arm.num_joints, _KD),
    tau=tau_g + _integral,
)
```

- `pos=_q_target`：目标关节角度（锁止或跟随）
- `vel=0`：目标关节速度为 0（阻尼）
- `kp=8.0`：位置增益（太大推不动，太小锁不住）
- `kd=1.5`：速度阻尼（抑制振荡）
- `tau=tau_g + _integral`：前馈力矩 = 重力补偿 + 积分修正

---

### 可调参数

| 参数 | 默认值 | 作用 | 调参建议 |
|------|--------|------|----------|
| `_VEL_THRESHOLD` | 0.04 m/s | 末端线速度阈值 | 太大 → 推很用力才响应；太小 → 轻微抖动就解锁 |
| `_W_VEL_THRESHOLD` | 0.08 rad/s | 末端角速度阈值 | 同上 |
| `_KP` | 8.0 | 位置增益 | 太大 → 推不动；太小 → 锁不住 |
| `_KD` | 1.5 | 速度阻尼 | 太大 → 阻尼感强；太小 → 振荡 |
| 积分增益 | 1.0 | 误差累积速度 | 太大 → 积分饱和快；太小 → 静态误差大 |
| 积分限幅 | ±0.5 N·m | 积分项上限 | 太大 → 积分饱和后抖动；太小 → 修正能力弱 |

---

### 与其他算法的对比

| 算法 | 目标角度 `q_target` | 行为 | 适用场景 |
|------|---------------------|------|----------|
| **09 号**（纯重力补偿） | 无（只有 `τ_g`） | 机械臂"飘"，微小扰动就漂移 | 零重力演示 |
| **10 号**（末端速度锁止） | 速度超阈值时更新 | 轻推能动、停手就锁 | 拖动示教、手动调姿 |

> 注：08 号是 `8_arm_traj_control.py`（轨迹控制），不是重力补偿算法。

---

### 学习收获

1. **末端速度作为判据**：比关节力矩或位置误差更直观，避免了力传感器的需求
2. **积分项的作用**：修正重力补偿的静态误差（URDF 参数不准、摩擦力），但需要限幅避免饱和
3. **雅可比矩阵的应用**：`v_spatial = J @ qd` 把关节空间速度映射到末端空间速度，是机器人学的核心工具
4. **MIT 模式的灵活性**：可以同时指定位置、速度、增益、前馈力矩，适合复杂控制律

---

### 下一步

- 跑一遍真机，体验"轻推能动、停手就锁"的效果
- 调参：尝试不同的 `_VEL_THRESHOLD` 和 `_KP`，找到最舒服的手感
- 扩展：在锁止状态下记录 `q_target` 序列，实现轨迹录制（示教编程）

---

## 2026-06-22：LeRobot AI 路线全景 + 两条路线辨析 + 模仿学习为啥"丝滑"

> 遥操作 60Hz 跑通后，跟自己捋清整条 AI 路线、两套体系的关系、以及模仿学习的核心原理。纯理解，不涉及操作。

### 一、整条 LeRobot AI 路线（我在哪 → 终点）

```
装机✅ → 烧录✅ → 重力补偿✅ → 102 leader✅ → 原生遥操作60Hz✅（我在这）
  → ① 装摄像头 → ② 采数据(50遍) → ③ 训练(ACT) → ④ 部署(AI自己跑) ═ 主线通关
  → ⑤ 深化(可选)：更多/更难任务、VLA 微调、语言控制
```
- **"采集+训练+部署"跑通一个任务 = 主线通关/里程碑**，证明走通了整套 LeRobot AI 工作流。之后是开放式深化，做多深自己定。
- **要不要走这条取决于目标**：只想手动遥控 → 遥操作通了就够；想让机械臂**自己干活** → 才需要采集+训练。
- 数据自己采（针对你的臂/任务，别人的不适配），但**模型可以用别人预训练的基础模型（π0/SmolVLA/GR00T）微调**，省数据。GPU 分工见 `遥操作与LeRobot待办.md` 阶段6。

### 二、两条独立路线：LeRobot(AI) vs ROS2(传统)——关键认知

项目里**两套东西不是一回事**，是机器人的两条独立路（详见 `AI机器人路径选择扫盲.md`）：

| | 🤖 LeRobot / AI（主线） | 🔧 ROS2 / MoveIt2（支线） |
|---|---|---|
| 怎么动 | AI 从示范**学**出来 | 工程师**写规则/规划**算出来 |
| 比方 | 你做几遍它看着悟 | 写好说明书+GPS 它照走 |
| 要训练/GPU | 要 | **不要**（确定性算法） |
| 擅长 | 视觉反应、灵巧、**举一反三** | **精确定位**、轨迹规划、碰撞避免 |
| 杀手锏 | **会看会反应**（闭环视觉，物体挪了能跟） | 精确可重复（但要已知目标，不会随机应变） |

- ⭐ **核心认知**：reBot-DevArm 是**练手载体/切入点**，真正学的是**可迁移的通用框架**（LeRobot 工作流 + ROS2 都 robot-agnostic）——学会了换 SO-100/工业臂/自己设计的机器人/找工作都能用，**不是只会玩这一台**。
- **学习策略**：两条都可跑 demo 理解全貌（广度），但**深挖先挑一条**（建议 LeRobot 主线深挖，ROS2 当以后可选副本，别同时精通免得乱）。ROS2 入手路径见待办 §平行支线2。

### 三、模仿学习为啥能"丝滑"（人遥操作数据是抖的，AI 学了为啥顺）

核心：**AI 不是"看一帧动一下"**（那样会抖），靠几个机制变顺——

1. **动作分块（Action Chunking）— 最核心**：ACT 全名 "Action Chunking with Transformers"，**一次预测未来一大段动作**（50~100 步）再平滑执行。比方：不是只看车头前 1 米猛打方向（抖），而是看着前方几秒规划整段轨迹（顺）。
2. **时间集成（temporal ensembling）**：每个时刻有多段重叠预测，**取加权平均**，把抖动磨平。
3. **从几十遍数据学"平均典型"动作**：单遍手抖，但学的是 50 遍共性，**噪声被平均掉**。
4. **高频连续控制 + 电机层插值**：30-60Hz 输出关节目标，MIT 模式平滑过渡。
- ⚠️ **诚实**：AI 不一定总丝滑，训练不够/数据太抖也会抖；丝滑是"好架构+够好数据"换来的，所以**采数据时自己遥操作要稳、动作干净**。
- **vs 路径规划**：规划=天生丝滑+精确**但死板**（要已知目标、不看环境）；AI=丝滑是学出来的**但会随机应变**（闭环看画面）。AI 的真正价值是"会反应"，丝滑是 chunking 顺带送的。这也是 ACT 当入门首选的原因——它就是为"平滑精细操作"设计的。

