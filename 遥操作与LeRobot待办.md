...
> 🟢 **2026-06-13 上游新增可参考脚本**：StarArm_102 上游（已同步到 `e8b871f`）加了 `Python_SDK/stararm102_ro_hover.py`（悬停）+ `stararm102_ro_mode.py`（hover+pinocchio 重力补偿+过温保护）。⚠️ 但**印证了本节结论**：hover 实现是"角度静止检测+位置锁+推动切 unlocked(0x10)"近似，**非真力矩**；想做的"读 current 电流估外力"和 `mode.py` 的 pinocchio RNEA 重力补偿路径**都被注释掉了（WIP）**。→ 将来想给 102 做力反馈/悬停，可拿这俩脚本的状态机当起点，但电流/动力学那块得自己接上。
...
### 3.7 官方 102 装配教程状态 ⚠️ 未发布（2026-06-01 查证；2026-06-11 复查仍无）

> 🟢 **2026-06-11 更新**：结构装配已自己完成。烧 ID 教程仓里也没有（Python_SDK README 只说"确认 ID 正确"、LeRobot 文档"校准"是 `lerobot-calibrate` 调限位非设 ID、FAQ 无）。已自建 [`102主动臂烧录指南.md`](./102主动臂烧录指南.md)，把从代码逆出的 ID↔关节对照 + 波特率 + 坑先填好，烧录步骤随实操丰富。
...
> ✅ **最终采购决定（2026-06-07）：两条路都玩，深度 + USB 都买。**
> - **Orbbec Gemini 2 已下单**（¥1750，天猫奥比中光官方旗舰店，3D 双目结构光）——官方 YOLO 抓取 demo 同款，仓里有现成腕部支架 `D435_Gemini2_Mount.step`
> - 普通 USB 摄像头（俯视用，型号待补）
> - 计划安装：Gemini 2 装**腕部**（满足抓取 demo 的 eye-in-hand 标定，其 RGB 流同时当 LeRobot 腕部相机）+ USB 当**俯视**
> - 下面的"原理论证"作为选型依据保留。
>
...
| **实际选择**：两条路都要 ← 最终走这条 | **Gemini 2（腕部，已下单）+ USB（俯视）** |
...
> ✅ **最终采购（2026-06-07）：Orbbec Gemini 2 已下单（¥1750，腕部，兼跑官方 YOLO 抓取 demo）+ 普通 USB 摄像头（俯视，型号待补）**。
>
> 选型原理（2026-06-01 查清，详见 §3.9）：LeRobot 的具身大模型（π0 / ACT / SmolVLA）只吃 RGB，深度只有传统 YOLO 抓取 demo 才用——最终决定两条路都玩，所以深度 + USB 都买。Gemini 2 的 RGB 流也作为 LeRobot 的腕部相机使用。
...
> ✅ **最终采购（2026-06-07）**：**Orbbec Gemini 2 已下单（¥1750，腕部）+ 普通 USB 摄像头（俯视，型号待补）**——两条路都玩。选型原理见 §3.9。

| 摄像头 | 价格 | 特点 | LeRobot 支持 | 用途 / 状态 |
|---|---|---|---|---|
| **Orbbec Gemini 2** | **¥1750 实付** | 深度+RGB，3D 双目结构光 | ✅ 一等公民 | ⭐ **已下单（2026-06-07，天猫官方旗舰店）**；装腕部，跑官方 YOLO 抓取 demo + 兼当 LeRobot 腕部 RGB |
| **普通 USB Webcam**（罗技 C270/C920） | ~¥100-300 | 只 RGB | ✅ OpenCV | ⭐ **俯视用（待补型号）** |
| 小 USB 摄像头模组（UVC 免驱，带长线） | ~¥30-80 | 只 RGB，小体积 | ✅ OpenCV | 备选（腕部已被 Gemini 2 占用，可作第三视角） |
| Intel RealSense D435/D435i | ~¥1500-2000 | 深度+RGB | ✅ 一等公民 | 不买（已选 Gemini 2） |
...
> 🟡 **ROS 侧参考（2026-06-11 上游社区贡献）**：@danieldoradotalaveron-rb 在主仓 README 贡献表加了 **D405 eye-in-hand TF**——在 `rebotarm_bringup` 里用 xacro 把 D405 腕部相机的坐标系（TF）挂到末端 `end_link` 下，让 RViz 能显示相机位姿。⚠️ **只有 TF/可视化，无相机驱动/深度流/内参标定，支架标定未完成**。跟你 Gemini 2 装腕部（eye-in-hand）是**同一思路**，等将来在 ROS2 里搞 eye-in-hand 视觉时可作"末端下挂相机 TF"的写法起点（注意它用 D405 不是 Gemini 2，且只是骨架不能直接用）。
...
### 平行支线 3：纯仿真遥操作（无硬件学习路径，候选）🟡

> ⭐ 来源：2026-06-11 上游同步进来的**社区贡献**（@danieldoradotalaveron-rb，挂在主仓 README "Contributions from Developers"）。代码在他个人 fork 的 `reBotArmController_ROS2`（ROS2 工作区）。**不是 Seeed 官方主线功能，实验/骨架阶段**。

#### 干嘛的

用**游戏手柄**控制机械臂**末端**在空间里移动，全程在 **RViz 仿真**里跑，**不用真机**：

- 核心是 **IK（逆运动学）**：推摇杆 = 给末端指方向，IK 实时反算 7 个关节各转多少度（对比直接控关节角的 FK，这个操作直觉得多）。
- 带 FK/IK + 安全措施 + RViz 实时显示机械臂状态。
- ⚠️ 贡献者明确标注 **simulation-only（仅仿真测试，没上真机）**。

#### 为什么值得收进待办

- 🟢 **降门槛**：整机自制 ~¥7500，这条路让"**没有机械臂也能学末端遥操作 + IK**"成立，是很好的入门/体验路径。
- 🟢 跟本仓**已有的"无硬件学习轨"是一条线**：`software/sim_extras/`（MeshCat 重力补偿 sim）、`fk_sim/ik_sim/traj_sim`、`rebotarm_moveit_demos`（MoveIt2 demo）—— 这条补上了之前缺的"**交互式遥操作**"体验。本质是 **sim-to-real / 数字孪生**思路。
- 🟢 **内容选题价值**：「没有机械臂也能学机械臂遥操作」对小红书是好钩子，虚拟→物理的对比叙事适合图文。

#### 前提条件（不是点一下就有）

| 前提 | 状态 |
|---|---|
| ROS2 Humble 工作区立起来（= 支线2 的前置） | ⚠️ 本仓 ROS2 至今"结构验证过、运行没跑过"，复刻这条 = 顺带把 ROS2 那摊子立起来 |
| 一个游戏手柄（Xbox/PS 类） | 🟡 零成本小硬件，待购 |
| 社区个人 fork 的实验代码 | ⚠️ 质量/维护无保证，大概率自己趟坑 |

#### 定位与时机

- **定位**：物理遥操作（102 leader → B601，见 §3.8）之前的**预热 + 内容选题**。先在 RViz 用手柄把"末端遥操作 + IK"手感跑通，再上真机 102 会更顺。
- **时机**：🟡 **候选，先占位不展开**。等 **102 装配（阶段3）告一段落**再决定要不要插进来，不打乱当前主线。
- 跟 102 leader 的关系：**两条平行的遥操作思路**——102 是"用一样的小臂带动大臂"（动作直接示范），这条是"手柄+逆解戳末端"。

---

### 平行支线 4：浏览器机械臂玩具 / Web GUI（降门槛选题，候选）🟡

> ⭐ 来源：B 站 up主「-万辞」（= GitHub **Yang-Ci**）的 reBot 二创开源仓 `github.com/Yang-Ci/Rebot-Arm`（CERN-OHL-W，2026-06-01）。已 clone 到本机 `F:\chengshenzhilu\Robot\Yang-Ci-Rebot-Arm`（独立目录，**不在主仓**）。详见记忆 [[reference-mujoco-web-sim]]。

#### 想法（用户 2026-06-29 提出）
两个价值：① **没机械臂的人也能玩**——浏览器里看 3D 臂、拖关节学，门槛砍到地板；② **一键 GUI vs 我们全程终端**——重力补偿/采集/训练现在全是终端敲命令，小白劝退，GUI 友好得多。正合「复刻 + 小白教程」定位。

#### 可行性（已扒代码，2026-06-29）🟢 近乎现成
他那仓的 `reBotArm_simulator/` 前端**本身就是个浏览器玩具**：
- `rebot-sim.js`(1635 行)：URDFLoader 浏览器内建模型 + `joint.setJointValue()` = **纯浏览器正运动学**；6 关节滑块（中文标签）+ 6 预设姿态（就绪/前方/左/右/检测/折叠）+ **拖末端解 IK**（戳工具尖自动够）+ 夹爪，**全本地**。
- ROS 是**独立可选层**（`rebot-ros-ui.js` 最后才叠）；**不点「连接 ROS」就是纯浏览器玩具**，只要 `npm start`（纯 Node 零依赖）发 URDF/STL，**不要 ROS2、不要硬件**。
- 改造成「零安装玩具」成本**低**：去掉/隐藏 ROS 连接 UI + 美化即可，难的（3D+FK+IK+预设）他都做完了，可直接借鉴他的 Three.js 前端（开源的正好是这块）。

#### ⚠️ 注意
- 视频（BV1BnJ569Eww，2026-06-12）里的 **MuJoCo 物理 + 颜色视觉抓取是他的新版，还没开源**（up主原话「之前的已经开源…这一版要多多打磨，后续也会同步开源推送」）。现开源仓 = 上一版（无 MuJoCo/视觉）。想要新版 → **关注他等推送**，或走官方 YOLO 抓取 demo（见 §3.9）。
- 真要做「一键重力补偿/采集/训练」GUI（包我们自己的流程）= 更重，后端（ROS2/LeRobot 环境）该装还得装，GUI 只是个好看的扳机，不消除安装坎。

#### 定位与时机
🟡 **候选，先占位不展开**。当前优先把 LeRobot 主线抓取成功率搞上去（补 50+ 条数据重训 ACT）。这个适合主线告一段落后，作为**降门槛 / 教程 / 小红书选题**展开（「没有机械臂也能在浏览器玩机械臂」是好钩子）。
